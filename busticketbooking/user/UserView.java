...
import console.busticketbooking.login.LoginView;
...
public class UserView implements UserViewCallback{
            ...
            try {
                ...
                switch (choice) {
                    case 1:
                        ...
                        break;
                    case 2:
                        userController.findAllBookings(user);
                        break;
                    case 3:
                        userController.getAllBooking(user);
                        break;
                    case 4:
    ...
    private void newBooking() {
        ...
        userController.findBus(from,to,date);
    ...
    public void showFoundBus(List<Bus> foundBus) {
        ...
        while (true) {
            try {
                ...
                userController.findSelectedBusSeats(selectedId,foundBus);
            } catch (Exception e) {
                System.out.println("Invalid input!");
            }
        }
        ...
        while(true) {
            ...
            for (int i = 0; i < bookings.size(); i++) {
                if (bookings.get(i).getBookingId() == cancelId) {
                    userController.cancelBooking(cancelId, user,bookings.get(i).getBus(),bookings.get(i).getPassengers());
                }
            }
    ...
    private void getPassangerDetails(Bus bus, int wantedSeats) {
        ...
        userController.addBooking(passengers,bus,user);
    }
